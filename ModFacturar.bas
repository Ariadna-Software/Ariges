...
Public Function TraspasoAlbaranesFacturas(cadSQL As String, cadWhere As String, FechaFact As String, banPr As String, ByRef PBarFac As ProgressBar, ByRef LblBar As Label, ImprimeLasFacturasGeneradas As Boolean, ByRef vTipoM As String, TextosCSB As String, NumeroCopias As Byte, MostrarMsgOK As Boolean, EsTraspasoOfeFAZ As Boolean, EsUnUnicoAlbaran As Boolean, FacturaSimplicadaSuperaImportesTicket As Boolean) As Boolean
    ...
    While Not RSalb.EOF
        ...
        '---------------------------------------------------------
            ...
            If cadW <> "" Then 'Facturacion pendiente
                cadW = cadW & ")) "
                If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, cadW, TextosCSB, ErroresAux, EsTraspasoOfeFAZ, False, False) Then
                    If B Then B = False
            ...
            If vParamAplic.ctaAportacion <> "" Then vFactu.Aportacion = DBLet(RSalb!Aportacion, "N")
            
            
                'aqui aqui
            If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, cadW, TextosCSB, ErroresAux, EsTraspasoOfeFAZ, EsUnUnicoAlbaran, FacturaSimplicadaSuperaImportesTicket) Then
                If B Then B = False
                AnyadirAvisos ErroresAux
            Else 'a?adirlo a la lista de facturas a imprimir
                If ListFactu = "" Then
        ...
        '----------------------------------------------------------
            ...
            Else
                ...
                Else
                    If vParamAplic.ctaAportacion <> "" Then vFactu.Aportacion = vFactu.Aportacion + DBLet(RSalb!Aportacion, "N")
...
'            antDirec = DBLet(RSalb!CodDirec, "N")
    ...
    If cadW <> "" Then
        ...
        'FacturaSuperaImporteTicket:  Solo la ultima puesto que solo estamos generadao facturas de una en una
        If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, cadW, TextosCSB, ErroresAux, EsTraspasoOfeFAZ, EsUnUnicoAlbaran, FacturaSimplicadaSuperaImportesTicket) Then
            If B Then B = False
...
Dim ListaFacturasDefinitiva As String
    ...
    Else
        ...
        If Sql = "" And listaF <> "" Then
            ...
            Else
                ...
                If FormatoFactura = 7 Then
                    'Si son proyectos EULER, vere si tiene lineas de impresion slifac_eu
                    devuelve = "codtipom='" & TipoFac & "' AND fecfactu=" & DBSet(fechaF, "F")
                    devuelve = devuelve & " AND numfactu IN (" & listaF & ") "
                    devuelve = "Select numfactu from slifac_eu2 WHERE " & devuelve
                    Set RN = New ADODB.Recordset
                    RN.Open devuelve, conn, adOpenForwardOnly, adLockPessimistic, adCmdText
                                    'NO tiene lineas de factura de impresion
                    If RN.EOF Then FormatoFactura = 0
    ...
    ElseIf FormatoFactura = 7 Then
        indRPT = 89  'Euler impresion lineas
    End If
        ...
        If vParamAplic.NumeroInstalacion = vbHerbelca Then
            If InStr(1, cadFormula, "FMO") > 0 Then
                ...
                If Val(devuelve) > 0 Then
                    'herFacturaPrecursor
                    devuelve = DevuelveDesdeBD(conAri, "documrpt", "scryst", "codcryst", "100")
    ...
    If TipoAlbaran <> "" Then
        ...
        Else
            ...
            Else
                ...
                ElseIf TipoAlbaran = "ALY" Then
                    DevuelveTipoDocumentoFactura = 7
                End If
            End If
        End If
    End If
...
Dim DarInternasContabilizadas As Boolean
    ...
    For J = 1 To 2
        ...
        While Not RSalb.EOF
                ...
                If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, Sql, "", ErroresAux, False, False, False) Then
                    B = False
                    AnyadirAvisos ErroresAux
                Else
...
Dim HazPulsarAceptarEnFrmImprimir As Boolean
    ...
    While Not RSalb.EOF
        ...
        '---------------------------------------------------------
            ...
            If cadW <> "" Then 'Facturacion pendiente
                cadW = cadW & ") "
                If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, cadW, TextosCSB, ErroresAux, False, False, False) Then
                    If B Then B = False
                    AnyadirAvisos ErroresAux
                Else 'a?adirlo a la lista de facturas a imprimir
...
Dim HazPulsarAceptarEnFrmImprimir As Boolean
    ...
    While Not RSalb.EOF
        ...
        actDirec = -1
             ...
             If condicion Then
                ...
                vFactu.DtoPPago = CCur(RSalb!DtoPPago)
                vFactu.DtoGnral = CCur(RSalb!DtoGnral)
                vFactu.Aportacion = DBLet(RSalb!Aportacion, "N")
             '   If vFactu.Aportacion = 0 Then Debug.Assert False
                'If RSalb!codtipom = "ALM" Then vFactu.Aportacion = DBLet(RSalb!Aportacion, "N")
                cadW = " (scaalb.codtipom,scaalb.numalbar) IN (('" & RSalb!codtipom & "'," & RSalb!Numalbar & ")"
                condicion = False
            Else
                vFactu.Aportacion = vFactu.Aportacion + DBLet(RSalb!Aportacion, "N")
                cadW = cadW & ",  ('" & RSalb!codtipom & "'," & RSalb!Numalbar & ")"
            End If
    ...
    If cadW <> "" Then
        ...
        If Not vFactu.PasarAlbaranesAFactura2(TipoAlb, cadW, "", ErroresAux, False, True, False) Then
            If B Then B = False
            AnyadirAvisos "Error Facturando el Cliente: " & Format(vFactu.Cliente, "000000") & " " & vFactu.NombreClien & vbCrLf & ErroresAux
        Else 'a?adirlo a la lista de facturas a imprimir
            ListFactu = ListFactu & "," & vFactu.Numfactu
