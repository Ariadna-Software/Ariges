...
Begin VB.Form frmFacCargos 
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   40
      TabIndex        =   0
      Tag             =   "cargo|T|N|||scargoscli|cargo||S|"
      Text            =   "Dat"
      Top             =   4920
      Width           =   4275
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmFacCargos.frx":000C
      Height          =   4725
      Left            =   120
      TabIndex        =   7
      TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmFacCargos"
...
Private Sub BotonBuscar()
    CargaGrid "cargo = '-1'"  'para vaciar los datos del Grid
    'Buscar
    txtAux(0).Text = ""
...
Dim SQL As String
    ...
    'Voy a comprobar si hay algun cargo
    SQL = DevuelveDesdeBD(conAri, "count(*)", "scliendp", " ucase(cargo)=" & DBSet(adodc1.Recordset!cargo, "T") & " AND 1", 1)
    If SQL = "" Then SQL = "0"
    If Val(SQL) > 0 Then
        MsgBox "Existen registros relacionados con el cargo", vbExclamation
        Exit Sub
    End If
    ...
    '### a mano
    SQL = "Va a eliminar el cargo:    " & adodc1.Recordset.Fields(0)
    SQL = SQL & vbCrLf & "¿Continuar?"
    
    If MsgBox(SQL, vbQuestion + vbYesNo) = vbYes Then
        'Hay que eliminar
        NumRegElim = Me.adodc1.Recordset.AbsolutePosition
        SQL = "Delete from scargoscli where  ucase(cargo)=" & UCase(DBSet(adodc1.Recordset!cargo, "T"))
        conn.Execute SQL
...
Private Sub Form_Load()
    ...
    'Cadena consulta
    CadenaConsulta = "Select * from scargoscli"
    CargaGrid
End Sub
...
Dim b As Boolean
    ...
    Else
        SQL = CadenaConsulta
    End If
    SQL = SQL & " ORDER BY cargo"
    
    CargaGridGnral DataGrid1, Me.adodc1, SQL, False

    'Nombre producto
    i = 0
        DataGrid1.Columns(i).Caption = "Cargos"
        DataGrid1.Columns(i).Width = DataGrid1.Width - 600
